...
class KLogger
{
    ...
    public function log($line, $severity, $args = self::NO_ARGUMENTS)
    {
        if ($this->_severityThreshold >= $severity) {
			...
			$lineToWrite = "$status $line";

            $name=AccountService::getInstance()->getCurrentAccountName();
            $memberId=AccountService::getInstance()->getCurrentMemberId();
            if($name !== null && $memberId !== null){
            	$memberString = "<" . $name . " (" . $memberId . ")>";
				$lineToWrite = "$status $memberString $line";
            }
            
            if($args !== self::NO_ARGUMENTS) {
                /* Print the passed object value */
                $lineToWrite = $lineToWrite . '; ' . var_export($args, true);
            }
            
            $this->writeFreeFormLine($lineToWrite . PHP_EOL);
        }
    }
